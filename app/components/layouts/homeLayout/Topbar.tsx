...
const Topbar: React.FC = () => {
  ...
  return (
    <div className="border-b w-full h-14 mt-1 flex items-center">
      <div className="flex gap-5 items-center ml-auto px-5">
        <button
          onClick={handleModal}
          className="bg-black hover:bg-gray-800 text-white text-xs py-2 rounded-md px-5 font-medium flex gap-1"
        >
          <PlusIcon className="w-4" />
          <span>Create</span>
        </button>
        <Modal isOpen={isOpen} onClose={handleModal}>
          <CreateBoardForm handleClose={handleModal} />
        </Modal>
